...
export function Sidebar() {
  ...
  return (
    ...
    >
      <div className="flex px-2 pt-2 gap-1 border-b" style={{ borderColor: 'var(--border)' }}>
        {TABS.map((tab) => (
          <button
            key={tab.kind}
            onClick={() => setActiveSource(tab.kind)}
            className="flex-1 text-xs px-2 py-2 rounded-t-lg cursor-pointer transition-colors truncate"
            style={{
